...
function getMatchResultFromScores(match) {
  if (!hasValidScore(match.scoreA) || !hasValidScore(match.scoreB)) return "";
...
function getSinglesNetScores(match) {
  ...
  const strokesA = Math.max(0, Number(match.hcpA || 0) - Number(match.hcpB || 0));
  const strokesB = Math.max(0, Number(match.hcpB || 0) - Number(match.hcpA || 0));
  return { netA: grossA - strokesA, netB: grossB - strokesB };
}
...
function getWinnerStyle(winner) {
  if (winner === "Europe") {
    return { border: "1px solid #93c5fd", background: "#dbeafe", color: "#1d4ed8" };
  }
  if (winner === "USA") {
    return { border: "1px solid #fca5a5", background: "#fee2e2", color: "#b91c1c" };
  }
  if (winner === "Halved") {
    return { border: "1px solid #cbd5e1", background: "#f8fafc", color: "#475569" };
  }
  return { border: "1px solid #e2e8f0", background: "#ffffff", color: "#64748b" };
}
...
export default function App() {
  ...
  const [nearestToHoleWinners, setNearestToHoleWinners] = useState(
    saved?.nearestToHoleWinners || Object.fromEntries(sideGameConfig.nearestToHoleHoles.map((hole) => [hole, { player: "", value: "" }]))
  );
  const [longDriveWinners, setLongDriveWinners] = useState(
    saved?.longDriveWinners || Object.fromEntries(sideGameConfig.longestDriveHoles.map((hole) => [hole, { player: "", value: "" }]))
  );
  ...
  const sidePoints = useMemo(() => {
    ...
    players.forEach((p) => {
      ...
      sideGameConfig.nearestToHoleHoles.forEach((hole) => {
        if (nearestToHoleWinners[hole]?.player === p.name) s += 1;
      });
      sideGameConfig.longestDriveHoles.forEach((hole) => {
        if (longDriveWinners[hole]?.player === p.name) s += 1;
      });
      pts[p.name] = s;
    });
    return pts;
  }, [players, sideStats, bestDriveWinner, nearestWinner, nearestToHoleWinners, longDriveWinners]);
  ...
  const leaderboard = useMemo(() => {
    return [...players]
      .map((p) => ({ ...p, points: Number(individualPoints[p.name] || 0) + Number(sidePoints[p.name] || 0) }))
      .sort((a, b) => b.points - a.points || a.handicap - b.handicap);
  }, [players, individualPoints, sidePoints]);

  const mobileMatches = useMemo(() => matches.filter((m) => m.simulator === mobileSimulator), [matches, mobileSimulator]);
  ...
  const updateMatch = (idx, field, value) => {
    setMatches((prev) => prev.map((m, i) => {
      ...
      if (field === "scoreA" || field === "scoreB") updatedMatch.winner = getMatchResultFromScores(updatedMatch);
      return updatedMatch;
    }));
  };
  ...
  const updateSideStat = (name, field, value) => {
    setSideStats((prev) => ({ ...prev, [name]: { ...(prev[name] || { bestDrive: "", nearestHole: "" }), [field]: value } }));
  };

  const updateHoleSideGame = (setter, state, hole, field, value) => {
    setter({ ...state, [hole]: { ...(state[hole] || { player: "", value: "" }), [field]: value } });
  };
  ...
  }, [players, matches, individualPoints, sideStats, tab, mobileSimulator, bestDriveWinner, bestDriveValue, nearestWinner, nearestValue, nearestToHoleWinners, longDriveWinners]);
  ...
  const resetSavedData = () => {
    ...
    setNearestWinner("");
    setNearestValue("");
    setNearestToHoleWinners(Object.fromEntries(sideGameConfig.nearestToHoleHoles.map((hole) => [hole, { player: "", value: "" }])));
    setLongDriveWinners(Object.fromEntries(sideGameConfig.longestDriveHoles.map((hole) => [hole, { player: "", value: "" }])));
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #f8fafc 0%, #e2e8f0 100%)", padding: 16, fontFamily: "Arial, sans-serif", color: "#0f172a" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gap: 16 }}>
        <div style={{ ...sectionStyle, padding: 0, overflow: "hidden" }}>
          <div style={{ background: "linear-gradient(90deg, #facc15, #fde68a)", padding: 14, textAlign: "center", fontWeight: 800, fontSize: 16, letterSpacing: 0.5, color: "#1f2937", borderBottom: "1px solid #f59e0b" }}>
          ...
          <div style={{ padding: 16 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
              ...
              <div style={{ border: "1px solid #e2e8f0", background: "#fff", borderRadius: 14, padding: 12 }}>
                ...
                <div style={{ fontSize: 28, fontWeight: 700 }}>{teamPoints.europe === teamPoints.usa ? "Tied" : teamPoints.europe > teamPoints.usa ? "Europe" : "USA"}</div>
        ...
        {tab === "matches" && (
          <div style={{ display: "grid", gap: 16 }}>
            {["Texas Scramble", "Fourball", "Singles (HCP)"].map((round) => {
              ...
              return (
                <div key={round} style={{ ...sectionStyle, border: theme.border, background: theme.background }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
                    <h2 style={{ margin: 0, color: theme.accent }}>{round}</h2>
                    <div style={{ padding: "6px 12px", borderRadius: 999, background: theme.chipBg, color: theme.accent, fontSize: 13, fontWeight: 700, border: theme.border }}>{matches.filter((m) => m.round === round).length} matches</div>
                  </div>
                  <div style={{ display: "grid", gap: 12 }}>
                    {matches.map((match, idx) => match.round === round ? (
                      <div key={`${round}-${idx}`} style={{ border: "1px solid #e2e8f0", borderRadius: 18, padding: 14, background: "#ffffff", boxShadow: "0 6px 18px rgba(15, 23, 42, 0.06)" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 8 }}>
                          <div style={{ fontSize: 13, color: "#64748b", fontWeight: 600 }}>{match.simulator}</div>
                          <div style={{ padding: "6px 10px", borderRadius: 999, fontSize: 12, fontWeight: 700, ...getWinnerStyle(match.winner) }}>{match.winner || "Awaiting score"}</div>
                        </div>
                        <div style={{ fontWeight: 800, fontSize: 18, margin: "4px 0 10px 0", color: "#0f172a" }}>{match.matchup}</div>
                        {round === "Singles (HCP)" && (
                          <div style={{ fontSize: 13, color: "#475569", marginBottom: 10, padding: 10, borderRadius: 12, background: "#ffffff", border: "1px dashed #93c5fd" }}>
                            ...
                            {hasValidScore(match.scoreA) && hasValidScore(match.scoreB) && (() => {
                              ...
                              return <div style={{ marginTop: 6, fontWeight: 700, color: "#1d4ed8" }}>Net: Europe {fmt(netA)} • USA {fmt(netB)}</div>;
                            })()}
                          </div>
                        )}
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
                          ...
                          <input style={{ ...inputStyle, border: "1px solid #93c5fd", background: "#eff6ff" }} placeholder="Europe side" value={match.scoreA} onChange={(e) => updateMatch(idx, "scoreA", e.target.value)} />
                          <input style={{ ...inputStyle, border: "1px solid #fca5a5", background: "#fef2f2" }} placeholder="USA side" value={match.scoreB} onChange={(e) => updateMatch(idx, "scoreB", e.target.value)} />
                        </div>
                        <div style={{ fontSize: 13, color: "#334155", marginBottom: 10, fontWeight: 600 }}>Auto result: {match.winner || "—"}</div>
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                          <button style={match.winner === "Europe" ? { ...primaryButton, background: "#2563eb", border: "1px solid #2563eb" } : buttonStyle} onClick={() => updateWinner(idx, "Europe")}>Europe win</button>
                          <button style={match.winner === "USA" ? { ...primaryButton, background: "#dc2626", border: "1px solid #dc2626" } : buttonStyle} onClick={() => updateWinner(idx, "USA")}>USA win</button>
                          <button style={match.winner === "Halved" ? { ...primaryButton, background: "#475569", border: "1px solid #475569" } : buttonStyle} onClick={() => updateWinner(idx, "Halved")}>Halved</button>
                        </div>
                      </div>
                    ) : null)}
                  </div>
                  ...
                  <div key={`${match.matchup}-${idx}`} style={sectionStyle}>
                    ...
                    {match.round === "Singles (HCP)" && (
                      <div style={{ fontSize: 13, color: "#64748b", marginBottom: 8 }}>
                        ...
                        {hasValidScore(match.scoreA) && hasValidScore(match.scoreB) && (() => {
                          const { netA, netB } = getSinglesNetScores(match);
                          return <div style={{ marginTop: 4 }}>Net: Europe {fmt(netA)} • USA {fmt(netB)}</div>;
                        })()}
                      </div>
                    )}
                    <div style={{ display: "grid", gap: 8 }}>
                    ...
                    <div style={{ fontSize: 13, color: "#475569", margin: "8px 0 0 0" }}>Auto result: {match.winner || "—"}</div>
        ...
        {tab === "sidegames" && (
          <div style={{ display: "grid", gap: 16 }}>
            <div style={sectionStyle}>
              ...
              <div style={{ color: "#475569", marginBottom: 12 }}>Course: <strong>{sideGameConfig.course}</strong></div>
              <div style={{ marginBottom: 18 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}>
                  {sideGameConfig.nearestToHoleHoles.map((hole) => (
                    <div key={`nth-${hole}`} style={{ border: "1px solid #bfdbfe", background: "#eff6ff", borderRadius: 14, padding: 12 }}>
                      <div style={{ fontWeight: 700, marginBottom: 8 }}>Hole {hole}</div>
                      <div style={{ display: "grid", gap: 8 }}>
                        <input style={inputStyle} value={nearestToHoleWinners[hole]?.player || ""} onChange={(e) => updateHoleSideGame(setNearestToHoleWinners, nearestToHoleWinners, hole, "player", e.target.value)} placeholder="Player" />
                        <input style={inputStyle} value={nearestToHoleWinners[hole]?.value || ""} onChange={(e) => updateHoleSideGame(setNearestToHoleWinners, nearestToHoleWinners, hole, "value", e.target.value)} placeholder="Distance, e.g. 1.82 m" />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}>
                  {sideGameConfig.longestDriveHoles.map((hole) => (
                    <div key={`ld-${hole}`} style={{ border: "1px solid #fecaca", background: "#fef2f2", borderRadius: 14, padding: 12 }}>
                      <div style={{ fontWeight: 700, marginBottom: 8 }}>Hole {hole}</div>
                      <div style={{ display: "grid", gap: 8 }}>
                        <input style={inputStyle} value={longDriveWinners[hole]?.player || ""} onChange={(e) => updateHoleSideGame(setLongDriveWinners, longDriveWinners, hole, "player", e.target.value)} placeholder="Player" />
                        <input style={inputStyle} value={longDriveWinners[hole]?.value || ""} onChange={(e) => updateHoleSideGame(setLongDriveWinners, longDriveWinners, hole, "value", e.target.value)} placeholder="Distance, e.g. 301 m" />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}>
                <div><div style={{ marginBottom: 6 }}>Best drive winner</div><input style={inputStyle} value={bestDriveWinner} onChange={(e) => setBestDriveWinner(e.target.value)} placeholder="Player" /></div>
                <div><div style={{ marginBottom: 6 }}>Best drive value</div><input style={inputStyle} value={bestDriveValue} onChange={(e) => setBestDriveValue(e.target.value)} placeholder="e.g. 289 m" /></div>
                <div><div style={{ marginBottom: 6 }}>Nearest hole overall winner</div><input style={inputStyle} value={nearestWinner} onChange={(e) => setNearestWinner(e.target.value)} placeholder="Player" /></div>
                <div><div style={{ marginBottom: 6 }}>Nearest hole overall value</div><input style={inputStyle} value={nearestValue} onChange={(e) => setNearestValue(e.target.value)} placeholder="e.g. 1.82 m" /></div>
              </div>
            </div>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ display: "grid", gap: 10 }}>
                ...
                <div style={{ display: "grid", gap: 4 }}>
                  {sideGameConfig.nearestToHoleHoles.map((hole) => (
                    <div key={`summary-nth-${hole}`}>Hole {hole}: {nearestToHoleWinners[hole]?.player || "—"} {nearestToHoleWinners[hole]?.value ? `• ${nearestToHoleWinners[hole].value}` : ""}</div>
                  ))}
                </div>
                <div><strong>Longest Drive holes:</strong> {sideGameConfig.longestDriveHoles.join(", ")}</div>
                <div style={{ display: "grid", gap: 4 }}>
                  {sideGameConfig.longestDriveHoles.map((hole) => (
                    <div key={`summary-ld-${hole}`}>Hole {hole}: {longDriveWinners[hole]?.player || "—"} {longDriveWinners[hole]?.value ? `• ${longDriveWinners[hole].value}` : ""}</div>
                  ))}
                </div>
